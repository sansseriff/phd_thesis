...
\textcolor{red}{ The following is very rough, taken from years ago when
I first started writing the manuscript }
...
the quantum channel \autocite{EraerdsRaman} \textcolor{red}{Calculate
...
high-count rate arrays have figures of merit of \textcolor{red}{FoM\_T
for Peacoq and Resta2023 results}. This would result in a proportinal
increase in the data rate. \textcolor{red}{FoM\_T for fastest InGaAs/InP
gated detector} These devices are ideal for fiber-based optical
communication. In free-space, the active area is especially important,
whithout the use of an adaptive optics system. \textcolor{red}{FoM\_T
for DSOC array}
...
\item
  ...
  \textcolor{blue}{ The formula for divergence angle in terms of waist
  \(w_0\): \(\theta = \frac{\lambda}{\pi w_0}\) }

  \textcolor{blue}{ Combining and plugging in, the waist radius at focus
  is
  \(\frac{1550~\mathrm{nm}}{\pi \tan^{-1}(\frac{3}{100})} \approx 16.5~ \mathrm{\upmu m}\)
  }

  \textcolor{blue}{ The formula for power inside an aperture at \(w(z)\)
  for a guassian beam:}
  ...
  \textcolor{blue}{We are interested in the ratio of power collected at
  \(w(z=0) = w_0\) which may be expressed as:}
...
\item
  ...
  \textcolor{blue}{ The waist radius at focus is
  \(\frac{1550~\mathrm{nm}}{\pi \tan^{-1}(\frac{3}{18})} \approx 2.98~\mathrm{\upmu m}\)
  }

  \textcolor{blue}{Ratio of power within the \(10~\mathrm{\upmu m}\)
  radius active area: }
...
\item
  ...
  \textcolor{blue}{ \textbf{Answer:} } \textcolor{blue}{ The three cases
  couple the same amount of light to the detector. (ii) couples the same
  amount of power as (i) because a blackbody source can't be focused to
  higher intensity with a lens. The solid angle subtended by the
  entrance pupil as seen by the detector is the same in all cases. The
  detector area stays the same as well so the etendue is conserved
  across all three cases. This implies the same radiant power is
  coupled. }

  \textcolor{blue}{ 3 points for saying all situations couple the same
...
\item
  ...
  \textcolor{blue}{ \textbf{Answer:} } \textcolor{blue}{The expression
  ...
  \textcolor{blue}{ Where \(P = \frac{\lambda}{hc}\) is the number of
  photons per unit energy, \(\Omega\) is the solid angle of blackbody
  radiation as seen by the detector, \(D_{area} = \pi r^2\) is the area
  of the detector, and \(B_{\lambda}\) is Planck's law. }
  \textcolor{blue}{Planck's law:}
  ...
  \textcolor{blue}{ Since the expression asked for can be written many
  ways, just verify the student has taken into account all the terms in
  equation (1) above, and has the correct expressions for}
  \textcolor{red}{\(\Omega\) (3 pts), \(\theta\) (3 pts), and P (3
  pts).}
\item
  ...
  \textcolor{blue}{ \textbf{Answer:} } \textcolor{blue}{Use the
  expression from (1.4) and multiply it by the quantum efficiency
  function \(\eta(\lambda)\) }
  ...
  \textcolor{blue}{ Here is the function expressed in mathmatica and the
  solution to the integral: }

  \textcolor{blue}{\includegraphics{chapter_05/figs_05/mathematica_2.PNG}}

  \textcolor{blue}{Dark count rate
  \(\approx \boxed{110 \,\text{MCounts/s}}\) } \textcolor{blue}{The rate
  of dark counts exceeds the usual maximum count rate, the detector is
  not usable. }

  \textcolor{blue}{3 pts. for similar dark count rate (+/- 20\%) , 3
...
\item
  ...
  \textcolor{blue}{\(\boxed{\text{3 filters}}\) are needed to make the
  wavelength band shorter than 156 nm the dominant source of counts.} {3
  pts. for this answer}

  \textcolor{red}{3 pts for evidence:} \textcolor{blue}{Students might
  integrate the detectable dark count spectrum with different numbers of
  filters and comparing the results. The computations below show the
  addition of a fourth filter has a negligible effect on the dark count
  rate. }

  \textcolor{blue}{Students may instead give a more qualitative answer,
  for example with a graph of the filtered spectrums, that shows the
  relative suppression of the region longer than 1560 nm. }

  \textcolor{blue}{\includegraphics{chapter_05/figs_05/filter_integrate_4.PNG}}
\item
  ...
  \textcolor{blue}{The expression for \(N_{photons}[\lambda]\) from part
  (1.4) can be simplified and evaluated at 1550 nm, then multiplied by
  the filter width in nanometers. }
  ...
  \textcolor{blue}{This code shows integrating the spectrum and just
  multiplying \(N_{photons}\) times the filter width produce very
  similar results (for a 1 nm filter):}

  \textcolor{blue}{\includegraphics{chapter_05/figs_05/nphoton_approx.PNG}}

  \textcolor{blue}{Evaluating the \(N_{photon}\) function at
  \(\lambda = 1550~text{nm}\) shows the \(-1\) term is small relative to
  the exponential term:}

  \textcolor{blue}{\includegraphics{chapter_05/figs_05/small_relative_to_exponential.PNG}}

  \textcolor{blue}{Therefore the filter transmission approximation is:}
  ...
  \textcolor{blue}{and the dark count rate in the 300 K room is roughly
  double the rate in the 293 K room: }

  \textcolor{blue}{\includegraphics{chapter_05/figs_05/filter_with_temp.PNG}}

  \textcolor{blue}{4 pts for a similar equation, 3 pts for finding the
...
\item
  ...
  \textcolor{blue}{For transform limited guassian pulses, the product of
  temporal and spectral width at a FWHM level is
  ...
  \textcolor{blue}{ Since this uses the FWHM level, all the 95\% metrics
  need to be converted. About 95\% of the area under a guassian falls
  within \(\pm 2 \sigma\). }

  \textcolor{blue}{ Bound on total system timing uncertainty:
  \(20~\text{ps}_{95\%} = (20/4)*2.35 = 11.75~\text{ps}_{FWHM}\) }

  \textcolor{blue}{ The jitter of the detection system and the temporal
  width of the laser pulse \(\Delta t\) should add in quadrature to
  match the bound: }
  ...
  \textcolor{blue}{ \(0.57~\text{nm}\) is the spectral width of the
  ...
  \textcolor{blue}{Dark count rate is easy to find using the expression
  ...
  \textcolor{blue}{3 points for writing and solving the equation that
  matches the jitter bound to the quadrature sum } \textcolor{blue}{5
...
\item
  ...
  \textcolor{blue}{ To lowest order in \(\gamma\),
  \(p \sim \gamma^{2}\eta\). The single photon fock state dominates for
  \(\gamma << 1\). }

  \textcolor{blue}{ 3 pts for correct \(p_{binary}(\gamma, \eta)\); 3
...
\item
  ...
  \textcolor{blue}{As \(\gamma\) approaches 1, the denominator in the
  original expression for \(\rho_s(\gamma,\eta)\) approaches infinity
  while the numerator approaches \(\eta\). In this limit, the SPDC is
  generating predominantly multi-photon states. For \(\gamma\)
  ...
  \textcolor{blue}{3 pts for correct \(F_{binary}(\gamma, \eta)\); 3 pts
...
\item
  ...
  \textcolor{blue}{The fist step is to determine the pump power
  ...
  \textcolor{blue}{\includegraphics{chapter_05/figs_05/Ftsolve.PNG}}

  \textcolor{blue}{Like in introductory statistics problems, its helpful
  to think about the negative case: Given N sources with herald
  ...
  \textcolor{blue}{Then the probability of at least one herald is 1
  ...
  \textcolor{blue}{\includegraphics{chapter_05/figs_05/mux_binary.PNG}}

  \textcolor{blue}{About \(\boxed{N = 290}\) sources are needed.}

  \textcolor{blue}{3 pts for correct form of the multiplexing expression
...
\item
  ...
  \textcolor{blue}{ \textbf{Answer:} } \textcolor{blue}{The POVM for one
  photon detected:}
  ...
  \textcolor{blue}{First, derive the probability of getting a single
  photon detection from the PNR detector: \(p_{PNR}(\gamma, \eta)\):}
  ...
  \textcolor{blue}{Where either of the boxed answers are acceptable, and
  the last line was found using mathematica:}

  \textcolor{blue}{\includegraphics{chapter_05/figs_05/simplify_ppnr.PNG}}

  \textcolor{red}{4 pts for \(p_{PNR}\left(\gamma, \eta\right)\)}

  \textcolor{blue}{Second, derive the single photon fidelity, staring
  with the density matrix for the signal photon given a PNR herald
  event. Using (15) above for
  ...
  \textcolor{blue}{4 pts for \(F_{PNR}(\gamma, \eta)\)}

  \textcolor{blue}{For \(\gamma\) near one, the SPDC is under strong
  pump power and is generating predominantly multi-pair states. A
  ...
  \textcolor{red}{4 pts for similar explanation}
\item
  ...
  \textcolor{blue}{ \textbf{Answer:} }
  \textcolor{blue}{\includegraphics{chapter_05/figs_05/PLT.PNG}}

  \textcolor{blue}{ The the herald probability regardless of idler arm
  efficiency is 25\%. }

  \textcolor{red}{ 4 points for the 25\% limit; 8 points for a few plots
  at different \(\eta\) }
\item
  ...
  \textcolor{blue}{ \textbf{Answer:} }
  \textcolor{blue}{\includegraphics{chapter_05/figs_05/pnrTotalPerf.PNG}}
  ...
  \textcolor{red}{ 4 points for each of the 2 answers. Answers that vary
  from these values by 2-3 sources are acceptable. }
\end{enumerate}
...
{ This text is blue. And it changes lightness when darkmode is witched }
