...
{Contact \href{mailto:andrewstermueller@gmail.com}{Andrew Mueller} with
any questions about the homework or solution manual. The solutions to
some sections specify finer-grained point values when there are multiple
...
\item
  ...
  { \textbf{Answer:} } { The divergence angle of the guassian beam:
  \(\theta = \tan^{-1}({\frac{3}{100}})\). }

  { The formula for divergence angle in terms of waist \(w_0\):
  \(\theta = \frac{\lambda}{\pi w_0}\) }

  { Combining and plugging in, the waist radius at focus is
  \(\frac{1550~\mathrm{nm}}{\pi \tan^{-1}(\frac{3}{100})} \approx 16.5~ \mathrm{\upmu m}\)
  }

  { The formula for power inside an aperture at \(w(z)\) for a guassian
  beam:}

  \[P(r, z)=P_{0}\left[1-e^{-2 r^{2} / w^{2}(z)}\right]\]

  {We are interested in the ratio of power collected at \(w(z=0) = w_0\)
...
\item
  ...
  { \textbf{Answer:} } { The three cases couple the same amount of light
  to the detector. (ii) couples the same amount of power as (i) because
  a blackbody source can't be focused to higher intensity with a lens.
  The solid angle subtended by the entrance pupil as seen by the
  detector is the same in all cases. The detector area stays the same as
  well so the etendue is conserved across all three cases. This implies
  the same radiant power is coupled. }

  { 3 points for saying all situations couple the same rate; 3 points
...
\item
  ...
  { \textbf{Answer:} } {The expression is a product of several factors:}

  \[\text{Flux}[\lambda] = P \Omega D_{area} B_{\lambda}(\lambda, T)\]

  { Where \(P = \frac{\lambda}{hc}\) is the number of photons per unit
  energy, \(\Omega\) is the solid angle of blackbody radiation as seen
  by the detector, \(D_{area} = \pi r^2\) is the area of the detector,
  and \(B_{\lambda}\) is Planck's law. } {Planck's law:}

  \[B_{\lambda}(\lambda, T)=\frac{2 h c^{2}}{\lambda^{5}} \frac{1}{e^{h c /\left(\lambda k_{\mathrm{B}} T\right)}-1}\]

  { \(\Omega = \pi \sin{\theta^2}\), where
  ...
  { Since the expression asked for can be written many ways, just verify
  the student has taken into account all the terms in equation (1)
  above, and has the correct expressions for} {\(\Omega\) (3 pts),
  \(\theta\) (3 pts), and P (3 pts).}
\item
  ...
  { \textbf{Answer:} } {Use the expression from (1.4) and multiply it by
  the quantum efficiency function \(\eta(\lambda)\) }

  \[N_{photons}[\lambda] = P \Omega D_{area} \eta(\lambda) B_{\lambda}(\lambda, T=293)\]

  { Here is the function expressed in mathmatica and the solution to the
  integral: }

  {\includegraphics{chapter_05/figs_05/mathematica_2.PNG}}

  {Dark count rate \(\approx \boxed{110 \,\text{MCounts/s}}\) } {The
  rate of dark counts exceeds the usual maximum count rate, the detector
  is not usable. }

  {3 pts. for similar dark count rate (+/- 20\%) , 3 pts. for saying the
...
\item
  ...
  {\(\boxed{\text{3 filters}}\) are needed to make the wavelength band
  shorter than 156 nm the dominant source of counts.} {3 pts. for this
  answer}

  {3 pts for evidence:} {Students might integrate the detectable dark
  ...
  {Students may instead give a more qualitative answer, for example with
  a graph of the filtered spectrums, that shows the relative suppression
  of the region longer than 1560 nm. }
...
\item
  ...
  {The expression for \(N_{photons}[\lambda]\) from part (1.4) can be
  simplified and evaluated at 1550 nm, then multiplied by the filter
  width in nanometers. }
  ...
  {This code shows integrating the spectrum and just multiplying
  \(N_{photons}\) times the filter width produce very similar results
  (for a 1 nm filter):}

  {\includegraphics{chapter_05/figs_05/nphoton_approx.PNG}}

  {Evaluating the \(N_{photon}\) function at \(\lambda = 1550~text{nm}\)
  shows the \(-1\) term is small relative to the exponential term:}
  ...
  {and the dark count rate in the 300 K room is roughly double the rate
  in the 293 K room: }

  {\includegraphics{chapter_05/figs_05/filter_with_temp.PNG}}

  {4 pts for a similar equation, 3 pts for finding the dark count rate
  roughly doubles. }
...
\item
  ...
  {For transform limited guassian pulses, the product of temporal and
  spectral width at a FWHM level is
  ...
  { Since this uses the FWHM level, all the 95\% metrics need to be
  converted. About 95\% of the area under a guassian falls within
  \(\pm 2 \sigma\). }

  { Bound on total system timing uncertainty:
  \(20~\text{ps}_{95\%} = (20/4)*2.35 = 11.75~\text{ps}_{FWHM}\) }

  { The jitter of the detection system and the temporal width of the
  laser pulse \(\Delta t\) should add in quadrature to match the bound:
  }
  ...
  { \(0.57~\text{nm}\) is the spectral width of the laser pulse at the
  FWHM level. If this pulse passes through a tophat filter with width
  equal to the 95\% level of the laser pulse, then 95\% will be
  transmitted. }
  ...
  {Dark count rate is easy to find using the expression from the
  previous section: }

  \[\boxed{N_{filter} \approx 9.26e12 (1~\text{nm}) e^{-9290/T} (\frac{\text{photons}}{\text{s*nm}})} \approx 0.15~\text{photons/s} \]

  {3 points for writing and solving the equation that matches the jitter
  bound to the quadrature sum } {5 points for correct filter width and
  dark count rate}
\end{enumerate}
...
\item
  ...
  { To lowest order in \(\gamma\), \(p \sim \gamma^{2}\eta\). The single
  photon fock state dominates for \(\gamma << 1\). }

  { 3 pts for correct \(p_{binary}(\gamma, \eta)\); 3 pts for saying the
  leading term is from single photons }
\item
  ...
  {As \(\gamma\) approaches 1, the denominator in the original
  expression for \(\rho_s(\gamma,\eta)\) approaches infinity while the
  numerator approaches \(\eta\). In this limit, the SPDC is generating
  predominantly multi-photon states. For \(\gamma\) approaching 1, the
  probability of the generated state being a single photon state goes to
  zero. Because the binary POVM was used, multi-photon states are
  `included' in \(\rho_s(\gamma,\eta)\). For \(\rho_s\) from the PNR
  POVM shown below, multi-photon states will be included to a much
  lesser extent, depending on the value for \(\eta\).}

  {3 pts for correct \(F_{binary}(\gamma, \eta)\); 3 pts for similar
  explanation}
...
\item
  ...
  {The fist step is to determine the pump power \(\gamma\) for which
  fideltiy is 98\%. }
  ...
  {Like in introductory statistics problems, its helpful to think about
  the negative case: Given N sources with herald probability \(p\), the
  probability of zero sources heralding is:}

  \[P(\text{no herald}|N) = (1 - p_{binary}\left(\gamma, \eta\right))^N\]

  {Then the probability of at least one herald is 1 minus the previous
  expression:}
  ...
  {About \(\boxed{N = 290}\) sources are needed.}

  {3 pts for correct form of the multiplexing expression
...
\item
  ...
  { \textbf{Answer:} } {The POVM for one photon detected:}

  \[\hat{\Pi}_{PNR}(1)=\sum_{n=1}^{\infty}n(1-\eta)^{n-1} \eta|n\rangle\langle n|\]

  {First, derive the probability of getting a single photon detection
  from the PNR detector: \(p_{PNR}(\gamma, \eta)\):}
  ...
  {Where either of the boxed answers are acceptable, and the last line
  was found using mathematica:}

  {\includegraphics{chapter_05/figs_05/simplify_ppnr.PNG}}

  {4 pts for \(p_{PNR}\left(\gamma, \eta\right)\)}

  {Second, derive the single photon fidelity, staring with the density
  matrix for the signal photon given a PNR herald event. Using (15)
  above for \(\langle \psi | \Pi_{\text {PNR}} | \psi \rangle\) in the
  denominator helps simplify it significantly. }
  ...
  {4 pts for \(F_{PNR}(\gamma, \eta)\)}

  {For \(\gamma\) near one, the SPDC is under strong pump power and is
  generating predominantly multi-pair states. A vanishing fraction of
  those states are single photon states that the PNR detector is able to
  distinguish and single-photon. Therefore, the PNR detector is
  signaling the generation of multi-pair states most of the time which
  should be discarded and do not contribute to \(p_{PNR}\). For high
  efficiency \(\eta\), only the vanishing single-pair creation rate
  contributes predominantly to \(p_{PNR}\).}
...
\item
  ...
  { \textbf{Answer:} } {\includegraphics{chapter_05/figs_05/PLT.PNG}}

  { The the herald probability regardless of idler arm efficiency is
  25\%. }

  { 4 points for the 25\% limit; 8 points for a few plots at different
  \(\eta\) }
\item
  ...
  { 4 points for each of the 2 answers. Answers that vary from these
  values by 2-3 sources are acceptable. }
\end{enumerate}
