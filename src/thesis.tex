...
50~ns. $\tilde{d}$ grows more dramatically with
decreasing $t^\prime$, especially in the range from 50~
to 100~ns. For count rates that don't exhibit many
inter-pulse arrival times smaller than 50~ns, a method
...
jitter when applied to detections from a 2.15~GHz pulse
...
deep-space optical
communication~\autocite{Srinivasan2023GroundReceiver} so
...
to have a ``~'' space in latex and a no break space in
