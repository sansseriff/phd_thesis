...
\begin{figure}
\centering
\includegraphics{chapter_05/figs_05/fig1_light.pdf}
\caption[{Cryostat concept designs.}]{\textbf{Cryostat concept designs}}
...
filters can be quite thick. Say a \(f = 100 mm\) lens is used right
outside the cryostat to focus the beam onto the detector though a set of
filters (Fig 1a). The long focal length makes room for a few inches of
filters between the external lens and focused spot.
...
\item
  ...
  \SI{5}{\micro\metre}, what ratio of power in the beam can it collect?
...
beam: \(\theta = \tan^{-1}({\frac{3}{100}})\). The formula for
divergence angle in terms of waist \(w_0\):
\(\theta = \frac{\lambda}{\pi w_0}\) Combining and plugging in, the
waist radius at focus is
...
\[P(r, z)=P_{0}\left[1-e^{-2 r^{2} / w^{2}(z)}\right]\] We are
interested in the ratio of power collected at \(w(z=0) = w_0\) which may
be expressed as: \[P(r, z=0)=1-e^{-2 r^{2} / w_0^{2}}\] Plugging in:
