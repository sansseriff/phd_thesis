...
As @eq:colorless is problematic for the narroband filtering regime, a conversion is needed that maps this type of expression to the more implicitly defined @eq:newmu. Luckily, the influence of narroband filtering as a sort of pseudo-loss can be rolled into a new geometric factor $\delta$. This is the ratio of the JSI captured by two narroband spectral filters $W_u(\lambda_s)*W_v(\lambda_i)$ to that captured by one $W_v(\lambda_i)$, as illustrated in @fig:narroband b and c. Consider an expression for $\delta$ in terms of measured and fitted quantities:
...
It is not trivially clear how much the interferometer early/late imbalances should should negatively impact entanglement visibility. The 2nd author of this paper, Samantha Davis, does a theoretical analysis to help answer this question, which is included <span class="latex"> in supplemental section \ref{Impact-of-experimental-imperfections-on-entanglement-visibility}</span><span class="html">[in the supplemental](../chapter_09/index.md)</span>. 
