...
The data representation and calibration curve shown in Fig. [1](#fig:slope_correction) a may be constructed with $t_B$ on the y-axis as well. Then the PNR corrections are applied the $t_B$ measurements instead. However, the resulting histograms $\tilde{t_B}$ are virtually identical to $\tilde{t}_A$ as they are ultimately constructed from the same data.
